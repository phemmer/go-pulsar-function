...
func NewClient(ctx context.Context, addr, namespace string) (*Client, error) {
	...
	// Ideally we shouldn't have to perform a Get request. We should just create and handle the NAMESPACE_EXISTS error.
	// However this doesn't work. Attempting to create a namespace that exists causes an internal server error.
	getReq := storage.GetNamespaceRequest{
		Name: namespace,
	}
	getResp, err := rootCl.GetNamespace(
		metadata.NewOutgoingContext(ctx, rootRangeMetadata),
		&getReq)
	if err == nil && getResp.Code == storage.StatusCode_SUCCESS {
		return cl, nil
	}

	crReq := storage.CreateNamespaceRequest{
		Name:                 namespace,
	}
	crResp, err := rootCl.CreateNamespace(
		metadata.NewOutgoingContext(ctx, rootRangeMetadata),
		&crReq)
	if err != nil { return nil, err }
	if crResp.Code != storage.StatusCode_SUCCESS && crResp.Code != storage.StatusCode_NAMESPACE_EXISTS {
		return nil, StorageError(crResp.Code)
	}
